...
@section('content')
    ...
    <div class="row">
        <div class="col-md-3">
            <a type="button" href="{{ route('equipamentos.update') }}" class="btn btn-sm btn-primary"><i class="fas fa-arrow-left"></i> Voltar</a>
        </div>
    </div>
    ...
    <div class="row mt-3">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('equipamentos.update') }}" method="post">
                        @csrf
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="nome_equipamento" id="nome_equipamento" class="form-control form-control-sm" value="{{ $equipamento->nome_equipamento }}">
                            </div>

                            <div class="col-md-3">
                                <label for="valor_equipamento">Valor do Equipamento <span class="text-danger">*</span></label>
                                <input type="text" name="valor_equipamento" id="valor_equipamento" class="form-control form-control-sm" value="{{ number_format($equipamento->valor_equipamento, 2, ',', '.') }}">
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="tipo_equipamento" id="tipo_equipamento" class="custom-select custom-select-sm" onchange="outro_tipo(this.value)">
                                    ...
                                    @foreach($tipo_equipamento as $key => $tipo)
                                        <option value="{{ $key }}" {{ ($equipamento->tipo_equipamento == $key) ? 'selected' : '' }}>{{ $tipo }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row mt-2">
                            <div class="col-md-3">
                                ...
                                <input type="text" name="outro_tipo_equipamento" id="outro_tipo_equipamento" class="form-control form-control-sm" {{ ($equipamento->tipo_equipamento != 11) ? 'readonly' : '' }} value="{{ $equipamento->outro_tipo_equipamento }}">
                            </div>

                            <div class="col-md-3">
                                <label for="tag_identificacao">TAG Identificação <span class="text-danger">*</span></label>
                                <input type="text" name="tag_identificacao" id="tag_identificacao" class="form-control form-control-sm" value="{{ $equipamento->tag_identificacao }}" disabled>
                            </div>

                            <div class="col-md-6">
                                <label for="detalhes">Detalhes</label>
                                <textarea name="detalhes" id="detalhes" cols="30" rows="1" class="form-control form-control-sm">{{ $equipamento->detalhes }}</textarea>
                            </div>
                        </div>
